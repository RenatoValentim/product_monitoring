Feature: Update Monitoring
  ...
  Scenario: Updating Monitoring with unavailable product
    ...
    And if "monitoring.alert 'mobile'", send a mobile alert notifying that the alert will be stopped
    And update "monitoring.alert = ''"
    Or if "monitoring.alert = 'email'", send an email alert notifying that the alert will be stopped
    and update "monitoring.alert = ''"
  ...
  Scenario: Updating Monitoring with updated product data
    ...
    And if "monitoring.alert = 'mobile'", send an mobile alert notifying about the product update
    Or if "monitoring.alert = 'email'", send an email alert notifying about the product update
